...
function App() {
  ...
  const listHook = () => {
    ...
    .then(newShowList => {
      ...
      const msgtToShow = ['success', 'Contact list loaded successfully!'];  
      return msgtToShow
/*       setMsgType('success');
    ...
    .then((msgtToShow) =>{
      // console.log('msgStyle, msgtext', msgtToShow[0], msgtToShow[1]);      
      setMsgType(msgtToShow[0]);
      setNotification(msgtToShow[1]);
      setTimeout(() => {
        setMsgType(null);
        setNotification(null);
      } ,3000)
  ...
  const deleteContact = (personId, persoName) => {
    ...
    if(window.confirm(`Do you want to delete contact ${persoName}?`)){
      ...
      .catch(error => {
        ...
        setMsgType('error');
        // setNotification(`Request Error: ${error}`);
        setNotification(`Information of ${persoName} has already been removed from server`);
        setTimeout(() => {
          setMsgType(null);
          setNotification(null);
        } ,3000) 
